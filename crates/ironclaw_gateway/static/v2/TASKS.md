...
This is the build plan for bringing `crates/ironclaw_gateway/static/v2` to parity with the legacy frontend in `crates/ironclaw_gateway/static`.
...
- Keep work scoped to `crates/ironclaw_gateway/static/v2` unless the backend API is missing.
...
```bash
node --check crates/ironclaw_gateway/static/v2/js/path/to/changed-file.js
git diff --check
```
