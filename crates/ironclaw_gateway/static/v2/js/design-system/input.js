...
export function Label({ children, className = "", required = false, ...rest }) {
  return html`
    ...
    >
      ${children}
      ${required && html`<span className="ml-0.5 text-[var(--v2-danger-text)]" aria-hidden="true"> *</span>`}
    </label>
  `;
}
...
export function FormField({
  ...
  return html`
    <div className=${cn("flex flex-col gap-2", className)}>
      ${label &&
        html`<${Label} htmlFor=${htmlFor} required=${required}>${label}<//>` }
      ${children}
      ${error &&
        html`<p className="text-xs text-[var(--v2-danger-text)]" role="alert">${error}</p>`}
      ${!error && hint &&
        html`<p className="text-xs text-[var(--v2-text-faint)]">${hint}</p>`}
    </div>
  `;
}
